...
template_1 = """<query>
我们需要诊断汽车子系统中是否存在某种症状。每个案例包含500个发动机噪声测量值的时间序列：<ts-data-blank-sep>，其中数据点以空格分隔，请问该案例中，汽车系统是否存在这种症状？
<response>
"""

list_templates_by_chatgpt = [
    """<query>
我们需要诊断汽车子系统是否存在某种症状。每个案例包括一个包含500个发动机噪声测量值的时间序列：<ts-data-blank-sep>，其中数据点以空格分隔。请问在该案例中，汽车系统是否存在这种症状？
<response>
""",
    """<query>
我们需要判断汽车子系统是否出现某种症状。每个案例都有一个包含500个发动机噪音测量值的时间序列：<ts-data-blank-sep>。数据点之间用空格分隔。请问在这个案例中，汽车系统是否存在这种症状？
<response>
""",
    """<query>
