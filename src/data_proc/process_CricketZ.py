...
template_1 = """<query>
下面给定一组时间序列：<ts-data-blank-sep>，它是从板球表演运动员手部佩戴的三维加速度传感器中获取的Z轴方向加速度数据。其中数据点采用空格间隔，加入了低方差白噪声。
请推断运动员可能正在进行的动作。
<options>
...
""",
    """<query>
The given set of time series, <ts-data-blank-sep>, represents the Z-axis acceleration data obtained from accelerometer sensors worn on the hand of a cricket performing athlete. The data points are separated by spaces and low variance white noise was appended.
...
""",
    """
<query>
The provided time series data, <ts-data-blank-sep>, corresponds to the Z-axis acceleration readings collected from accelerometer sensors placed on the wrist of a cricket player during their performance. The data points are separated by spaces and have been augmented with low variance white noise.
